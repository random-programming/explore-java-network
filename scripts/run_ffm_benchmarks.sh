...
# Run FFM-only benchmark matrix (reduced: skip 10000 connections — single-threaded server)
# 1 model × 4 conn × 8 sizes × 3 cpu × 2 runs = 192 tests
...
TOTAL=$((${#CONNECTIONS[@]} * ${#DATA_SIZES[@]} * ${#CPU_CONFIGS[@]} * RUNS))
CURRENT=0
SKIPPED=0
FAILED=0
START_TIME=$(date +%s)
...
echo "================================================================"
echo "IO Benchmark — FFM io_uring Tests"
echo "Model: $MODEL"
...
send_telegram "🚀 *FFM Benchmark started*
        ...
        for size in "${DATA_SIZES[@]}"; do
            for run in $(seq 1 $RUNS); do
                ...
                if [ "$AVAIL_GB" -lt 20 ]; then
                    ...
                    send_telegram "⛔ *FFM Benchmark STOPPED*
