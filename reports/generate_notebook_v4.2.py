#!/usr/bin/env python3
"""Generate benchmark analysis notebook v4.2 — fixed charts (CPU grouped bars, FFM annotations, FD section marked)."""
...
"""# Анализ бенчмарка I/O моделей Linux/Java (v4.2)
...
**Изменения в v4.2 (относительно v4):**
- **Исправлен график CPU** (секция 5): вместо stacked bar — grouped bar с чёткой легендой (модели + conn)
- **Добавлены аннотации FFM** на графиках, где однопоточный FFM отсутствует (ring overflow при >10 conn)
- **Секция FD помечена** как содержащая некорректные данные (collector считал FD Gradle-процесса, а не сервера)
...
> **ВНИМАНИЕ: данные в этой секции некорректны.** Collector (`MetricsCollector.java`) считал FD для PID Gradle-процесса, а не сервера.
> Все модели показывают ~180 FD при любом числе соединений — это FD Gradle daemon (classpath, JIT, temp).
> Ожидаемое поведение: FD ≈ connections + baseline (~50). Для исправления необходимо починить `find_java_pid()` в `collect_metrics.sh` и пересобрать метрики.
...
- **Все модели** показывают одинаковое количество FD.
- **FFM** может показывать меньше FD при >10 conn (не успевает принять все соединения).
- **FFM-MT** должен показывать количество FD аналогичное Netty-моделям (принимает все соединения).
- FD не зависят от размера данных и числа ядер.
"""
))
...
nb.cells = cells
out_path = '/ssd/benchmark/reports/benchmark_analysis_v4.2.ipynb'
