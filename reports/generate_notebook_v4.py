#!/usr/bin/env python3
"""Generate benchmark analysis notebook v4 with 6 I/O models (including io_uring FFM-MT)."""
...
"""# Анализ бенчмарка I/O моделей Linux/Java (v4)
...
**Изменения в v4 (относительно v3):**
- Добавлена 6-я модель: **io_uring FFM-MT** — многопоточный io_uring-сервер на базе Panama FFM API
- FFM-MT архитектура: acceptor thread + N worker threads, каждый со своим io_uring ring (4096 entries), fixed buffers, SQPOLL
- Теперь возможно **честное сравнение** FFM-MT vs Netty io_uring (JNI) при всех уровнях conn
...
nb.cells = cells
out_path = '/ssd/benchmark/reports/benchmark_analysis_v4.ipynb'
