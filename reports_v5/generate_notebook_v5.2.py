#!/usr/bin/env python3
"""Generate benchmark analysis notebook v5.2 — CS/CPU по всем потокам (/proc/PID/task/*/status), защита от отрицательных delta."""
...
- **Итого: 280 тестов, 0 ошибок, 355 мин**

**Ключевые исправления v5.2 (относительно v5):**
- **Context switches:** теперь суммируются из `/proc/PID/task/*/status` (все потоки процесса). В v5 CS читались только из `/proc/PID/status` (главный поток) и были нулевыми. Теперь видны реальные данные: FFM-MT 16K-38K/sec, Blocking 89K-209K/sec.
- **CPU:** суммируется из `/proc/PID/task/*/stat` (все потоки) для более точных значений.
- **Защита от отрицательных delta:** `max(0, delta)` при пересоздании потоков / race condition.
- **Аномалия:** io_uring JNI при 100 conn — run1=8,852 RPS vs run2=39,425 RPS (cold start нестабильность).

**Исправления v5 (относительно v4.3):**
- **PID сервера:** через `ss -tlnp`, а не Gradle wrapper.
- **CPU/CS:** delta-расчёт вместо кумулятивных значений.
...
RESULTS_DIR = Path('/ssd/benchmark/results_v5.2')
...
**Ключевое отличие v5.2:** CPU и context switches суммируются по всем потокам процесса (`/proc/PID/task/*/stat` и `/proc/PID/task/*/status`). Защита от отрицательных delta.
"""
))
...
**Исправления v5.2:** CS суммируются из `/proc/PID/task/*/status` (все потоки). В v5 читался только главный поток — значения были нулевыми. Теперь видны реальные данные.
"""
))
...
### Рейтинг моделей I/O (по совокупности метрик, v5.2)
...
| 3 | **io_uring (FFM-MT)** | Лидер при 100+ conn, **5-9x меньше CS**, fixed buffers | JDK 21+, custom event loop |
...
### Context Switches — новые данные v5.2

В v5 CS были нулевыми (читался только главный поток). В v5.2 CS суммируются из `/proc/PID/task/*/status` (все потоки):

| Модель | 1 conn | 100 conn | 1000 conn | 10000 conn |
|--------|--------|----------|-----------|------------|
| **FFM-MT** | 16K | 22K | 38K | 18K |
| **io_uring JNI** | 83K | 132K | 116K | 97K |
| **Blocking** | 89K | 209K | 209K | 128K |
| **Epoll** | 84K | 160K | 138K | 143K |
| **NIO** | 101K | 133K | 155K | 126K |

FFM-MT имеет в **5-9x меньше voluntary CS** благодаря io_uring batching и меньшему числу syscalls.

### Аномалия io_uring JNI

io_uring JNI при 100 conn показывает нестабильность cold start: run1=8,852 RPS vs run2=39,425 RPS (4.5x). Не наблюдается у других моделей. Вероятная причина — JIT + native transport init.
...
- **Результат:** FFM-MT при 100+ conn превосходит Netty io_uring (JNI) по throughput благодаря fixed buffers и оптимизированному ring (4096 entries).
...
6. **io_uring JNI cold start** — run1 при 100 conn может показывать до 4.5x ниже RPS.
"""
))

nb.cells = cells
out_path = '/ssd/benchmark/reports_v5/benchmark_analysis_v5.2.ipynb'
